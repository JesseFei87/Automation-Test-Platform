...
- 当前行级 AI 按钮只能执行 `promoted_case_id` 对应的正式 YAML；未转正式草稿没有 `test-cases/icm/*.yaml` 文件，runner 无法直接定位。
- 直接自动转正式会绕过 YAML 质量门禁，并可能污染正式 case 资产；本轮采用“临时执行草稿 YAML”更安全。
- 后续如果草稿运行 passed，再由用户决定是否转正式或合并 observed asset，保持“真实通过后再沉淀”的资产原则。
