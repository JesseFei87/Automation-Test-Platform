...
const admin = { username: process.env.ICM_ADMIN_USERNAME || "admin", password: process.env.ICM_ADMIN_PASSWORD || "" };
...
await page.getByText("设备信息", { exact: true }).click().catch(async () => {
  await page.getByRole("link", { name: "设备信息", exact: true }).click();
});
await page.waitForLoadState("networkidle").catch(() => {});
...
| --- | --- | --- | --- |
| TC-ICM-003 | ICM device list query | ${status} | Opened \`Device Ops > Device List\`, queried \`${queryKeyword}\`, and stayed on the device list view. |
