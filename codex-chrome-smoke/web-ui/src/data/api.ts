...
export const api = {
  ...
  analyzeRequirementSpec: (title: string, document: string) =>
  ...
  createRun: (mode: "run-case" | "run-batch", caseId?: string) =>
    request<{ id: string; mode: "run-case" | "run-batch"; case_id: string | null; status: string }>("/runs", {
      method: "POST",
      body: JSON.stringify({ mode, case_id: caseId }),
    }),
