...
async def run(page, system, case) -> None:
    ...
    if await page.locator('tbody tr').filter(has_text='ABCDEFGHIJKLMNO').count():
        await ensure_text_visible(page, 'ABCDEFGHIJKLMNO')
        return
    ...
    await inputs.nth(2).fill('ABCDEFGHIJKLMNO')
