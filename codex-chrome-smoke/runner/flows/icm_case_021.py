...
async def run(page, system, case) -> None:
    ...
    await search_by_keyword(page, SEARCH_NAME_SELECTORS, "TestDev_01")
    if await page.locator("tbody tr").filter(has_text="TestDev_01").count():
        await ensure_text_visible(page, "TestDev_01")
    ...
    await inputs.nth(2).fill("TestDev_01")
    ...
    remark_field = dialog.locator(
        "textarea:visible, input[placeholder*='备注']:visible, textarea[placeholder*='备注']:visible"
    ).first
    ...
    await settle(page, 1200)
    await search_by_keyword(page, SEARCH_NAME_SELECTORS, "TestDev_01", settle_ms=1200)
    await ensure_text_visible(page, "TestDev_01")
    await ensure_text_visible(page, "192.168.1.100")
    await ensure_text_visible(page, "在线")
