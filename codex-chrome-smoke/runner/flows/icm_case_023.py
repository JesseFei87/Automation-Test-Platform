...
async def run(page, system, case) -> None:
    ...
    if await page.locator('tbody tr').filter(has_text='BCDEFGHIJKLMNOP').count():
        await ensure_text_visible(page, 'BCDEFGHIJKLMNOP')
        return
    ...
    await inputs.nth(2).fill('BCDEFGHIJKLMNOPQ')
